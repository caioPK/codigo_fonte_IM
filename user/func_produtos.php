<?php
	...
	if ($rows>0)
	{
	   ...
	   $administrativo2=$campo->SalAdm;   
	   $administrativo3=$campo->MediAdmi;
	   
	   $demaisfunc1=$campo->DemaFunc;
	   $demaisfunc2=$campo->SalaDema;
	   $demaisfunc3=$campo->MediDema;
	   
	   $salmedio=$campo->SalMedio;   
	   $opermaq=$maquina1+$maquina2+$maquina3+$maquina4+$maquina5+$maquina6+$maquina7+$operariosmont;	  
	   $total1=round($total_par*$opermaq);// somatorio do numero de funcionarios
	   $total2=$operarios3+$administrativo3+$demaisfunc3+$montagem3;
