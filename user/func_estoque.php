<?php
	...
	$select="SELECT * FROM resultado WHERE Jogo='$codjogo' AND Jogada='$jogada' AND Equipe='$CodEquipe' LIMIT 1"  ;
	...
	for($cont=1; $cont<=$rows; $cont++)
	{
	  ...
	  $TotalContasPagar=$FGTS+$IRPJ+$INSS+$ICMS+$PIS+$COFINS+$IRPJ+$DesNaoOperacional;
	}     
	...
	{   
	  $select="SELECT * FROM produto WHERE Jogo='$codjogo' AND Jogada='$jogada' AND Equipe='$codequipe' ORDER BY Produto"  ;
	  $resultado=mysqli_query($con, $select);
	  $rows=mysqli_num_rows($resultado);
	  for($cont=1; $cont<=$rows; $cont++)
	  {
	    $campo=mysqli_fetch_object($resultado);
	    $Estoqueant[$cont]=intval($campo->Estoque);
	...
	$Producao[1]=$Qtdereal[1];
	$Producao[2]=$Qtdereal[2];
	$Producao[3]=$Qtdereal[3];
	$Producao[4]=$Qtdereal[4];
