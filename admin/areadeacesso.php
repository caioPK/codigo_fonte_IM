<?php
  ...
  if ($sele == "")
  {
    $select = "SELECT * from equipe e, jogos j WHERE j.CodJogo = '".$_SESSION['codjogo']."' and e.CodJogo = j.CodJogo ORDER BY CodEquipe ASC Limit 1";
    $pegaequipe = mysqli_query($con, $select);
    $campo = mysqli_fetch_object($pegaequipe);
    $jogada = $campo->Jogada;
    $fase = $campo->FaseJogo;
    if ($fase==2) {
  ...
  elseif($sele != "")
  {
      ...
      if ($jogada>0) {
        ...
        $select = "SELECT * from equipe WHERE CodEquipe = '$codequipe'";
        $pegaequipe = mysqli_query($con, $select);
        $campo = mysqli_fetch_object($pegaequipe);
        $nome = $campo->NomeEquipe;
        $codjogo = $_SESSION['codjogo'];
...
<html lang="en-gb">
  ...
  <head>
    ...
    <script type="text/javascript" src="//ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js"></script> <!-- SCRIPT PARA FAZER A TRANSIÇÃO ENTRE ABAS -->
      ...
      <div class="textousuario" style="padding-bottom: 10rem;">
        <h3> 
          <?php
              if ($jogada<=0)
              {
                echo "Não houve jogada ainda. <br> <div style='margin-bottom: 4rem;'></div>";
              }
              else
              {
                echo "Jogada ".$jogada." da equipe ".$nome.""; 
              }     
         ?>
       </h3>
       <?php if ($jogada >= 1){ ?>
        <h5 style="color: #324252; display: inline-block; margin-right: 1rem; font-size: 1rem;"> Selecione jogada e equipe:</h5>
        <form method="post" action="" style="display: inline-block;">    
          <select name="jogada" class="select-wrapper" style="width: 13rem; margin-right: 1rem;">
            <?php
              ...
              {  
                ...
                if ($sele !="") 
                {
                    if($cont==1)
                    {
                      echo '<option value="'.$jogada.'"> Jogada '.$jogada.'</option>';
                      $rows=$cont; 
                    }
                    elseif($cont == $jogada)
                    {
          ...
          <select name="equipe" class="select-wrapper">
            <?php

              if($sele != "")
              {
                $select="SELECT * FROM equipe WHERE CodJogo='".$_SESSION['codjogo']."' ORDER BY CodEquipe='$codequipe' DESC";
                $resultado=mysqli_query($con, $select);
                $linhas = mysqli_num_rows($resultado);
                for($cont=1; $cont<=$linhas; $cont++)
                {  
                  $campo=mysqli_fetch_object($resultado);
                  echo "<option value=$campo->CodEquipe> $campo->NomeEquipe </option>" ;
                }
                if($rows==0)
                {
                  echo '<option value="'.$cont.'"> Nenhuma equipe no jogo </option>'; 
                }
              }
              else
              {
          ...
          <button type="submit" name="selequi" value="selecionar" style="display: inline-block;margin-top:-0.1px;;margin-left: 1rem; width: 9rem; height: 1.7rem;">Selecionar</button>
        </form>
                ...
                <div id="tab1" class="tab active">
                  <div align=center>
                    ...
                    <table class="tabusuario">     
                      ...
                      <tr> 
                        <td><p>- Custos de BackOrder</p></td>
                        <td><p>'.$CustoBackOrder.'</p></td>
                      </tr>
                      <tr> 
                        <td><p>- Qualidade</p></td>
                        <td><p>'.$Qualidade.'</p></td>
                ...
                <div id="tab3" class="tab">
                  <div align=center>
                    ...
                    <table class="tabusuario">     
                      ...
                      <tr> 
                        <td><p>- Custos de Back Order </p></td>
                        <td><p></p></td>
                        <td><p>'.$CustoBackOrder.'</p></td>
                        <td><p>'.$saldo[24].'</p></td>
                      </tr>
                ...
                <div id="tab5" class="tab">
                  <div align=center>
                    ...
                    <table class="tabusuario">     
                      ...
                      <tr> 
                        <td><p>Hora Extra</p></td>
                        <td><p></p></td>
                        <td><p></td>
                        <td><p>'.$CustoHoraExtra.'</p></p></td>
                      </tr>
                      <tr> 
                        <td><p>Totais</p></td>
                        <td><p>'.$total1.'</p></td>
