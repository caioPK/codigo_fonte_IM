<?php
	...
	if($jogada > 1){
		$Construcoes = 0; $Equipamentos = 0;
		for ($i=1; $i <= $jogada ; $i++) { 
			$select="SELECT CaixaConstrucoes, CaixaEquipamentos FROM resultado WHERE Jogo='$codjogo' AND Jogada='$i' AND Equipe='$codequipe'";
			$resultado=mysqli_query($con, $select);
			$campo=mysqli_fetch_object($resultado);
			$Construcoes = $campo->CaixaConstrucoes;
			$Equipamentos = $campo->CaixaEquipamentos;
		}
	}else{
		$select="SELECT Construcoes, Equipamentos FROM resultado WHERE Jogo='$codjogo' AND Jogada='$jogada' AND Equipe='$codequipe'";
		$resultado=mysqli_query($con, $select);
		$campo=mysqli_fetch_object($resultado);
	...
	$select="SELECT * FROM resultado WHERE Jogo='$codjogo' AND Jogada='$jogada' AND Equipe='$codequipe'" ;
		...
		if (mysqli_num_rows($resultado)>0)
		{
			...
			$TotalReal=$Estoque+$Aplicacao+$Caixa+$DuplicatasReceber;
		...
		if(true)
		{
			...
			$select="select Receita from resultado WHERE Jogo='$codjogo' AND Jogada='".($jogada-1)."' AND Equipe='$codequipe' ";	
			$resultado=mysqli_query($con, $select);  
			$campo=mysqli_fetch_object($resultado);
		...
		$Soma1=$AtivoCirculante+$Construcoes+$Equipamentos;
		
		$Soma3 = $TotalEx + $Capital + $LucroAcumulado;
